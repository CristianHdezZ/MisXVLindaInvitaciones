...
const LOCAL_CONFIG_FILE = '/tmp/site-config.json';
...
async function getConfig() {
  ...
  try {
    const raw = await fs.readFile(LOCAL_CONFIG_FILE, 'utf8');
    return JSON.parse(raw);
  } catch {
    return null;
  }
...
async function setConfig(config) {
  if (hasRedis) {
    ...
    await fs.writeFile(LOCAL_CONFIG_FILE, JSON.stringify(config), 'utf8');
  }
}

module.exports = { appendRsvp, listRsvps, getConfig, setConfig, hasRedis };
